/*
 * nixio2 - Linux I/O library for lua supplement for nixio
 *
...
static void delete_from_monitor_list(monitor *m) {
	if (m->next == m) {
		monitors = NULL;
		DBG("delete %s. last monitor",m->key);
	} else {
		if (m == monitors) {
			monitors = m->next;
		}
...
static void send_http(monitor *m, char *command, char *hexbuff, regmatch_t *matches, char *error_message) {
	char http[1000];
	int hlen = snprintf(http, sizeof(http), "GET /data_request?id=action&DeviceNum=%d&serviceId=urn:gengen_mcv-org:serviceId:ZWaveMonitor&action=%s&key=%s",
						m->device_num, command, m->key);
	if (matches) {
		int j;
		int start = 1, end = 9;
...
static void process_zwave(int input_fd, int output_fd, int send, zwave_state *s) {
	...
	if ((raw_len = read(input_fd, raw_buff, sizeof(raw_buff))) > 0) {
		...
		while (p < endp) {
			...
			if (send && kill_acks && s->state == 0) {
				if (c == 6) {	/* ack */
					startp = p;
					DBG("   Swallowed an ack due to kill_acks=%d", kill_acks);
					if (kill_acks == 2) {
						int rlen = s->response_zpos-s->response_midpos;
						DBG("   Writing second part of response: %d bytes", rlen);
						int result = write(input_fd, s->response_midpos, rlen);
						if (result < 0) {
							thread_error("Intercept write part 2", errno);
						}
						kill_acks = 1;
					}
					continue;
											...
											if(m->response) {
												...
												while ((c = *r++) && !error) {
													...
													if (c >= '0' && c <= '9' && rstate < 4) {
														...
														monitor_error(m, "Syntax error in response");
														error = 1;
														break;
													}
													switch(rstate) {
														...
														case 3: /* \1 - \9 replacement */
															if (matches[val].rm_so < 0) {
																monitor_error(m, "More replacements than matches");
																error = 1;
															} else {
														...
														case 5: /* Checksum */
															if (s->response_zstate < 2) {
																monitor_error(m, "Response too short for checksum");
																error = 1;
															} else if (addZwaveResponseData(m, s,&s->response_checksum, 1)) {
																error = 1;
															} else {
																s->response_zpos[1] = s->response_zstate-2; /* Set the length byte */
															}
															if (!s->response_midpos) {
																s->response_midpos = s->response_zpos;
															}
												...
												if (!error) {
													int rlen = s->response_zpos - s->response_buff; 
													if (send) {
														if (s->response_midpos && s->response_midpos < s->response_zpos) {
														 	rlen = s->response_midpos - s->response_buff;
															kill_acks = 2; 
														} else {
															kill_acks = 1;
														}
...
static void *zwint_threadFunction(void *arg) {
	...
	while(1) {
		...
		while (monitors && monitors->timeout && monitors->timeout <= ms) {
			...
			if (pollfds[2].revents != 0) {
				...
				if (pollfds[2].revents & POLLIN) {
					...
					do {
						...
						if (len2 > 0) {
							buffer[len2] = 0;
							DBG("Received from http server: %s", buffer);
						}
#endif
						if (len2 == 0 && first) {
							close(http_fd);
