...
SID_SCENECONTROLLER   = "urn:gengen_mcv-org:serviceId:SceneController"
...
Devices = {
	...
	[DEVTYPE_EVOLVELCD1] = {
	    ...
	    ServiceId               = "urn:gengen_mcv-org:serviceId:EvolveLCD1",
		...
		SetTuningParameters = function(zwave_dev_num)
			...
			local retry = 0
			while retry < 10 do
				versionString = luup.variable_get(ZWDEVICE_SID, "VersionInfo", zwave_dev_num)
	...
	[DEVTYPE_COOPEREFWC5] = {
	    ...
	    ServiceId               = "urn:gengen_mcv-org:serviceId:CooperRFWC5",
		...
		SetTuningParameters = function(zwave_dev_num)
			...
			GetTuningParameter("SCENE_CTRL_UseWithSlaveController"  , 0             , zwave_dev_num)
			GetTuningParameter("SCENE_CTRL_Retries"       		    , 0             , zwave_dev_num)
			GetTuningParameter("SCENE_CTRL_AssociationDelay"        , 3500          , zwave_dev_num)
			GetTuningParameter("SCENE_CTRL_SceneControllerConfDelay", 3250          , zwave_dev_num)
		end,
	...
	[DEVTYPE_NEXIAONETOUCH] = {
	    ...
	    ServiceId               = "urn:gengen_mcv-org:serviceId:NexiaOneTouch",
...
BINARY_DEVTYPE     = "urn:schemas-upnp-org:device:BinaryLight:1"
DIMMABLE_DEVTYPE   = "urn:schemas-upnp-org:device:DimmableLight:1"
THERMOSTAT_DEVTYPE = "urn:schemas-upnp-org:device:HVAC_ZoneThermostat:1"

HADEVICE_SID       = "urn:micasaverde-com:serviceId:HaDevice1"
HAG_SID            = "urn:micasaverde-com:serviceId:HomeAutomationGateway1"
SCTRL_SID          = "urn:micasaverde-com:serviceId:SceneController1"
ZWN_SID       	   = "urn:micasaverde-com:serviceId:ZWaveNetwork1"
ZWDEVICE_SID       = "urn:micasaverde-com:serviceId:ZWaveDevice1"
DIMMING_SID        = "urn:upnp-org:serviceId:Dimming1"
FANMODE_SID        = "urn:upnp-org:serviceId:HVAC_FanOperatingMode1"
USERMODE_SID       = "urn:upnp-org:serviceId:HVAC_UserOperatingMode1"
SWITCHPOWER_SID    = "urn:upnp-org:serviceId:SwitchPower1"
TEMPSENSOR_SID     = "urn:upnp-org:serviceId:TemperatureSensor1"
COOLSETPOINT_SID   = "urn:upnp-org:serviceId:TemperatureSetpoint1_Cool"
HEATSETPOINT_SID   = "urn:upnp-org:serviceId:TemperatureSetpoint1_Heat"
...
function EnqueueZWaveMessage(name, device, node_id, data, delay)
  EnqueueActionOrMessage({type=1, name=name, device=device, node_id=node_id, data=data, delay=delay})
end

function EnqueueLuupAction(name, device, service, action, arguments, delay)
  EnqueueActionOrMessage({type=2, name=name, device=device, node_id=-1, service=service, action=action, arguments=arguments, delay=delay})
end
...
function RunInternalZWaveQueue(fromWhere)
  ...
  while true do
	...
	if j.pending then
		VLog("RunInternalZWaveQueue canceled due to pending job")
		return
	end
	if not j.slept then
		j.slept = 0
	...
	if j.slept == 0 then
		if j.type == 1 then
			if j.node_id > 0 then
				VLog("RunInternalZWaveQueue: type=ZWave, Node=Device name="..j.name..": " .. ZWN_SID .. " SendData " .. VTableToString({Node = j.node_id, Data = j.data}) .. " " .. ZWaveNetworkDeviceId);
				j.err_num, j.err_msg, j.job_num, j.arguments = luup.call_action(ZWN_SID, "SendData", {Node = j.node_id, Data = j.data}, ZWaveNetworkDeviceId)
			else
				VLog("RunInternalZWaveQueue: type=ZWave, Node=Controller name="..j.name..": " .. ZWN_SID .. " SendData " .. VTableToString({Data = j.data}) .. " " .. ZWaveNetworkDeviceId);
				j.err_num, j.err_msg, j.job_num, j.arguments = luup.call_action(ZWN_SID, "SendData", {                  Data = j.data}, ZWaveNetworkDeviceId)
			end
		else
	...
	if luup.job_watch then
		...
		VLog("RunInternalZWaveQueue: Setting job:"..j.name.." to pending state");
		j.pending = true
		return
	end
	if j.delay < 100 then
	...
	while not j.completed and j.slept < 60000 and (j.err_num < 2 or j.err_num > 4) do
	...
	if j.delay >= 1000 then
	  local seconds = math.floor(j.delay / 1000);
	  j.delay = j.delay % 1000
	  j.completed = true
	  VLog("delaying again for " .. seconds .. " seconds. " .. j.delay .. " ms remain.")
	  luup.call_delay("RunInternalZWaveQueue", seconds, fromWhere, true)
	  return	
	end
	if j.delay > 0 and ZWaveQueueNext.next == ZWaveQueueNext then
      luup.sleep(j.delay)
	end
...
function ClearScreen(dev_num, node_id)
	if SCObj.HasScreen then
		EnqueueZWaveMessage("ClearScreen", dev_num, node_id, "146 2 0 0 0 0 0 0 0", param.SCENE_CTRL_ClearDelay);
	end
end
...
function EVLCDLabel(screenFlags,lineArray, node_id, zwave_dev_num, delay_override)
	...
	if param.SCENE_CTRL_CharDelay == nil then
		SetTuningParameters(zwave_dev_num)
	end
	...
	for lnum = 1,#lineArray do
        ...
        delay = delay + #line[3] * param.SCENE_CTRL_CharDelay 
   		if part == 1 and lnum < #lineArray and param.SCENE_CTRL_MaxParts > 1 then
			...
			EnqueueZWaveMessage("SetLabel_"..node_id, zwave_dev_num, node_id, data, delay);
	...
	for zwave_dev_num,v in pairs(luup.devices) do
		...
		if candidate and v.device_num_parent == 1 then
			...
			if peer_dev_num ~= nil and peer_dev_num > 0 and luup.devices[peer_dev_num] == nil then
		   		...
		   		luup.call_action(HAG_SID,"DeleteDevice", {DeviceNum = zwave_dev_num}, 0);
	...
	if v2.device_type == SCObj.DEVTYPE and v2.device_num_parent == 0 then
		...
		if v == nil then
			...
			SceneIdCacheList[zwave_dev_num] = nil
		   	luup.call_action(HAG_SID,"DeleteDevice", {DeviceNum = peer_dev_num}, 0);
		else 
			...
			luup.variable_watch("SceneController_LastSceneIDChange",SCTRL_SID,"LastSceneID",zwave_dev_num)
			luup.variable_watch("SceneController_LastSceneTimeChange",SCTRL_SID,"LastSceneTime",zwave_dev_num)
			if IsVeraPrimaryController() then
				luup.variable_watch("SceneController_ConfiguredChanged",HADEVICE_SID,"Configured",zwave_dev_num)
				if param.SCENE_CTRL_Retries > 0 then
...
if not j.delay then
	ELog("j.delay is NULL - j="..tableToString(j))
end
	local sleep = j.delay % 1000
	local job_delay = j.delay - sleep
	if j.delay > 0 then
		VLog("SceneController_JobWatchCallBack: Delaying for " .. j.delay .. "ms. sleeping for " .. sleep .. "ms. Job delay =" .. job_delay .. "ms.")
	end
	if sleep > 0 then
		luup.sleep(j.delay)
	end
...
function SetConfigurationOption(name, dev_num, node_id, option, value, delay)
    ...
    EnqueueZWaveMessage("SetConfigurationOption("..name..":"..tostring(option).."="..tostring(value)..")"..node_id, dev_num, node_id, data, delay)
end
...
function SetIndicatorValue(peer_dev_num, indicator, force, delay)
	if SCObj.HasIndicator then
		...
		if force or tonumber(previous_indicator_string) ~= indicator then
			EnqueueZWaveMessage("SetIndicator("..indicator..")"..node_id, peer_dev_num, node_id,  "0x87 0x01 "..indicator, delay);
		...
		for button = 1, 5 do
			...
			if numLines == 5 or ((button ~= 1 or scrollOffset == 0) and (button ~= SCObj.NumButtons or scrollOffset == numLines-SCObj.NumButtons)) then
				...
				elseif mode.prefix == "X" then
			   		local scene = luup.variable_get(SCTRL_SID, "sl_SceneActivated", peer_dev_num)
					if scene then
		   ...
		   IsPrimaryController = true
			...
			for k,v in pairs(luup.devices) do
				if v.device_type == "urn:schemas-micasaverde-com:device:ZWaveNetwork:1" then
					local role = luup.variable_get(ZWN_SID, "Role", k)
...
function UpdateActConfList(actConfList, oldSceneId, newSceneId, controller_dev_num, target, virtualButton, level, scren, act)
	if oldSceneId ~= newSceneId then
		...
		if newSceneId then
			...
			EnqueueZWaveMessage("SelectSceneId_SceneActuatorConf("..newSceneId..","..level..","..target.dimmingDuration..")"..target_node_id, target.device, target_node_id,  "44 1 ".. newSceneId .. " ".. target.dimmingDuration .. " 128 ".. level, 0);
		end
	end
end
...
function SetDeviceActuatorConf(controller_dev_num, target_dev_num, target_node_id, sceneId, button, level, dimmingDuration)
	...
	EnqueueZWaveMessage("SceneActuatorConf("..sceneId..","..level..","..dimmingDuration..")"..target_node_id, target_dev_num, target_node_id,  "44 1 ".. sceneId .. " ".. dimmingDuration .. " 128 ".. level, 0);
	...
	if force or prevModeStr ~= modeStr then
		...
		if not mode.cooperConfiguration then
			...
			if force or sceneId ~= prevSceneId then
				...
				EnqueueZWaveMessage("SetSceneControllerConfig_On("..tostring(physicalButton)..","..sceneId..")"..node_id, zwave_dev_num, node_id,  "0x2d 0x01 "..physicalButton.." "..sceneId.." 0xFF", param.SCENE_CTRL_SceneControllerConfDelay)
			end
			if SCObj.HasOffScenes and mode.prefix == "T" then
				...
				if force or offSceneId ~= prevOffSceneId then
					-- Set scene controller configuration, group id, sceneid, dimming Duration
					EnqueueZWaveMessage("SetSceneControllerConfig_Off("..(physicalButton+SCObj.NumButtons)..","..offSceneId..")"..node_id, zwave_dev_num, node_id,  "0x2d 0x01 "..(physicalButton+SCObj.NumButtons).." "..offSceneId.." 0xFF", param.SCENE_CTRL_SceneControllerConfDelay)
				end
			end
			if mode.sceneControllable then
				for i = 1, #mode do
...
function SetTemperatureLCDParameters(zwave_dev_num, temperatureDevice)
	...
	if temperatureDevice > 0 then
		temperatureString = luup.variable_get(TEMPSENSOR_SID, TEMPSENSOR_VAR, temperatureDevice)
	end
	if temperatureString ~= nil then
		...
		EnqueueZWaveMessage("SetDisplayTemperature", zwave_dev_num, lcd_node_id,  "0x31 0x5 0x1 0x9 ".. temperature, 0)  
	end 
	...
	local userMode = luup.variable_get(USERMODE_SID, USERMODE_VAR, temperatureDevice)
	if userMode == USERMODE_COOL then
		local coolSetPointString = luup.variable_get(COOLSETPOINT_SID, SETPOINT_VAR, temperatureDevice)
		if coolSetPointString ~= nil then
			...
			EnqueueZWaveMessage("SetDisplaySetpoint", zwave_dev_num, lcd_node_id, "0x43 0x03 0x02 0x9 ".. coolSetPoint, 0) 
		end 
	elseif userMode == USERMODE_HEAT then
		local heatSetPointString = luup.variable_get(HEATSETPOINT_SID, SETPOINT_VAR, temperatureDevice)
		if heatSetPointString ~= nil then
			...
			EnqueueZWaveMessage("SetDisplaySetpoint", zwave_dev_num, lcd_node_id, "0x43 0x03 0x01 0x9 ".. heatSetPoint, 0) 
		end 
	else
		if temperatureString ~= nil then
			local temperature = math.floor(tonumber(temperatureString) + 0.5);
			-- COMMAND_CLASS_THERMOSTAT_SETPOINT THERMOSTAT_SETPOINT_REPORT  SetPoint = Heating 1 Level = Size=1 | Scale=1 | Precision=0
			EnqueueZWaveMessage("SetDisplaySetpoint", zwave_dev_num, lcd_node_id, "0x43 0x03 0x01 0x9 ".. temperature, 0) 
		end 
	end

	local fanModeString = luup.variable_get(FANMODE_SID, FANMODE_VAR, temperatureDevice)
	if fanModeString ~= nil then
		...
		EnqueueZWaveMessage("SetDisplayFanMode", zwave_dev_num, lcd_node_id, "0x44 0x3 ".. fanMode, 0) 
	end 
end
...
function UnassociateDevice(zwave_dev_num, target_dev_num_list, firstGroup, lastGroup, groupStep)
	...
	for group = firstGroup, lastGroup, groupStep do
		EnqueueZWaveMessage("RemoveAssociation("..group..")"..zwave_node_id.."-x->"..label, zwave_dev_num, zwave_node_id,  "0x85 0x04 "..group..targets, param.SCENE_CTRL_AssociationDelay)
	end
end
...
function AssociateDevice(zwave_dev_num, target_dev_num_list, firstGroup, lastGroup, groupStep)
	...
	-- COMMAND_CLASS_ASSOCIATION ASSOCIATION_REMOVE
	for group = firstGroup, lastGroup, groupStep do
		EnqueueZWaveMessage("SetAssociation("..group..")"..zwave_node_id.."->"..label, zwave_dev_num, zwave_node_id, "0x85 0x01 "..group..targets, param.SCENE_CTRL_AssociationDelay)  
	end
end
...
function SetReturnRoute(zwave_dev_num, lcd_node_id, target_node_id)
	...
	if target_node_id ~= veraZWaveNode then
		...
		if not  SceneController_ReturnRoutes[lcd_node_id][target_node_id] then
			EnqueueZWaveMessage("AssignReturnRoute_"..lcd_node_id.."->"..target_node_id, zwave_dev_num, 0,  "0x00 0x46 ".. lcd_node_id .. " " .. target_node_id .. " " .. veraZWaveNode, param.SCENE_CTRL_ReturnRouteDelay)
...
function TemperatureDeviceChanged2(part, peer_dev_num, temperatureDevice, service, variable, old_val, new_val)
	...
	if service == HEATSETPOINT_SID and variable == SETPOINT_VAR then
		...
		local target = luup.variable_get(HEATSETPOINT_SID, SETPOINT_TARG, temperatureDevice)
		local temperatureString = luup.variable_get(TEMPSENSOR_SID, TEMPSENSOR_VAR, temperatureDevice)
		...
		if target ~= new_val and (userMode == USERMODE_COOL) then
			EnqueueLuupAction(HEATSETPOINT_SID, temperatureDevice, "SetCurrentSetpoint", {NewCurrentSetpoint = old_val}, 2000)
			EnqueueLuupAction(COOLSETPOINT_SID, temperatureDevice, "SetCurrentSetpoint", {NewCurrentSetpoint = new_val}, 2000)
	...
	if service == USERMODE_SID and variable == USERMODE_VAR then
		if new_val == USERMODE_COOL then
			local coolSetpoint_string = luup.variable_get(COOLSETPOINT_SID, SETPOINT_VAR, temperatureDevice)
			newCoolSetPoint = math.floor(tonumber(coolSetpoint_string) + 0.5);
		else
			local heatSetpoint_string = luup.variable_get(HEATSETPOINT_SID, SETPOINT_VAR, temperatureDevice);
			newHeatSetPoint = math.floor(tonumber(heatSetpoint_string) + 0.5);
		end
		redraw = false;
	end

	if service == TEMPSENSOR_SID and variable == TEMPSENSOR_VAR then
		local temperature = math.floor(tonumber(new_val) + 0.5);
		-- COMMAND_CLASS_SENSOR_MULTILEVEL SENSOR_MULTILEVEL_REPORT Sensor Type = Temperature Level = Size=1 | Scale=1 | Precision=0
		EnqueueZWaveMessage("SetDisplayTemperature", peer_dev_num, lcd_node_id, "0x31 0x5 0x1 0x9 ".. temperature, 2000)
	end

	if service == FANMODE_SID and variable == FANMODE_VAR then
		local fanMode = 1
		if 	new_val == FANMODE_AUTO then
			fanMode = 0
		end
		-- COMMAND_CLASS_THERMOSTAT_FAN_MODE  THERMOSTAT_FAN_MODE_REPORT
		EnqueueZWaveMessage("SetDisplayFanMode", peer_dev_num, lcd_node_id, "0x44 0x3 ".. fanMode, 2000)
	end
...
function TemperatureDeviceChanged3(peer_dev_num, setpoint, heatOrCool,temperatureDevice)
	...
	-- COMMAND_CLASS_THERMOSTAT_SETPOINT THERMOSTAT_SETPOINT_REPORT  SetPoint = Heating 1 Level = Size=1 | Scale=1 | Precision=0
	EnqueueZWaveMessage("SetDisplaySetpoint", peer_dev_num, lcd_node_id, "0x43 0x03 1 0x9 ".. setpoint, 0)
	AssociateDevice(zwave_dev_num, {[temperatureDevice]=true}, 2, 4, 1);
end
...
function SetTemperatureScreen(peer_dev_num, screenNum, timeout, forceClear, indicatorOnly)
	...
	if screenNum > SCObj.NumTemperatureScreens and curTemperatureDevice > 0 then
		...
		if WatchedTemperatureDevices[curTemperatureDevice] == nil then
			...
			luup.variable_watch("TemperatureDeviceChanged", TEMPSENSOR_SID,   TEMPSENSOR_VAR,   curTemperatureDevice) 
			luup.variable_watch("TemperatureDeviceChanged", COOLSETPOINT_SID, SETPOINT_VAR,     curTemperatureDevice) 
			luup.variable_watch("TemperatureDeviceChanged", HEATSETPOINT_SID, SETPOINT_VAR,     curTemperatureDevice) 
			luup.variable_watch("TemperatureDeviceChanged", FANMODE_SID,      FANMODE_VAR,      curTemperatureDevice) 
			luup.variable_watch("TemperatureDeviceChanged", USERMODE_SID,     USERMODE_VAR,     curTemperatureDevice)
		end 
	end
end
...
function SceneController_LastSceneIDChange(lul_device, lul_service, lul_variable, lul_value_old, lul_value_new)
	...
	local peer_dev_num_str = luup.variable_get(SCObj.ServiceId,"PeerID",zwave_dev_num)
	local peer_dev_num = tonumber(peer_dev_num_str)
	local zwave_scene_str, scene_id_update_time = luup.variable_get(SCTRL_SID, "LastSceneID",zwave_dev_num) 
...
function SceneController_LastSceneTimeChange(lul_device, lul_service, lul_variable, lul_value_old, lul_value_new)
	...
	local peer_dev_num_str = luup.variable_get(SCObj.ServiceId,"PeerID",zwave_dev_num)
	local peer_dev_num = tonumber(peer_dev_num_str)
	local scene_activated_str, scene_activated_update_time = luup.variable_get(SCTRL_SID, "sl_SceneActivated", zwave_dev_num) 
	local scene_activated = tonumber(scene_activated_str)
	local zwave_scene_str, scene_id_update_time = luup.variable_get(SCTRL_SID, "LastSceneID",zwave_dev_num) 
	local zwave_scene = tonumber(zwave_scene_str)
	local lastTime = SceneTimes[peer_dev_num]
	local lastScene = SceneIds[peer_dev_num]
	if scene_activated == 0 then
		SceneChange(peer_dev_num, 0, scene_id_update_time)
		return 
	elseif lastTime and lastTime >= scene_id_update_time and lastScene == zwave_scene  then
		return;
	end 
...
function SceneChange(peer_dev_num, zwave_scene, cur_scene_time) 
	...
	if zwave_scene == 0 then
		...
		if not physicalButton then
			ELog("SceneChange: TODO. Read back indicator")
			return
		end
	...
	local virtualButton = physicalButton + scrollOffset
	DLog("SceneChange: act_deact=" .. act_deact .. " activate=" .. tostring(activate) .. " physicalButton=" .. physicalButton .. " currentScreen=" .. currentScreen .. " numLines=" .. numLines .. 
	...
	if peer_scene ~= nil then
		...
		luup.variable_set(SCTRL_SID, act_deact, tostring(peer_scene), peer_dev_num)
		local peer_scene_time_str = luup.variable_get(SCTRL_SID, "LastSceneTime", peer_dev_num)
		...
		luup.variable_set(SCTRL_SID, "LastSceneTime", tostring(cur_scene_time), peer_dev_num)
...
function GetDeviceStatus(device_num)
  ...
  if device.device_type == BINARY_DEVTYPE then
    local service = SWITCHPOWER_SID
  ...
  if device.device_type == DIMMABLE_DEVTYPE then
    local service = DIMMING_SID
...
function SetDeviceStatus(device_num, value)
  ...
  elseif device.device_type == DIMMABLE_DEVTYPE then
    service = DIMMING_SID
	variable = "LoadLevelStatus"
	value = tostring(value)
  else
	DLog("  SetDeviceStatus: "..tostring(device.device_type).." not one of "..tostring(BINARY_DEVTYPE).." or "..tostring(DIMMABLE_DEVTYPE));
    return
  end
