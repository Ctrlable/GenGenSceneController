...
DEVTYPE_ZWAVEMONITOR  = "urn:schemas-gengen_mcv-org:device:SceneControllerInstaller:1"

SID_SCENECONTROLLER   = "urn:gengen_mcv-org:serviceId:SceneController"
SID_ZWAVEMONITOR      = "urn:gengen_mcv-org:serviceId:SceneControllerInstaller"
...
ZWAVEMONITOR_SID   = "urn:schemas-gengen_mcv-org:service:SceneControllerInstaller"
...
-- pattern is a Lua pattern which will be matched against the incoming Z-Wave data.
-- Callback is a function which is passed the peer device number and the capture
-- from the lua pattern. The capture is nil if a timeout occurred.
-- If callback is nil, then this can be used to wait until the response is recevice
-- without regard to its value.
-- Timeout and delay are in milliseconds. Delay is applied after the response is received.
-- but ignored in case of a timeout.
function EnqueueZWaveMessageWithResponse(name, node_id, data, delay, pattern, callback, timeout)
  local context
  if callback then
  	ResponseContextNum = ResponseContextNum + 1
  	context = "X" .. ResponseContextNum
  	ResponseContextList[context] = callback
  end
  EnqueueActionOrMessage({
  	type=1,
  	name=name,
...
function SceneController_ZWaveMonitorResponse(device,response,context)
	local now = socket.gettime()
	VLog("SceneController_ZWaveMonitorResponse: device="..tostring(device).." response="..tostring(response).." context="..tostring(context))
	if context then
		local callback = ResponseContextList[context]
...
zwaveMonitor = nil
function GetZWabeMonitor()
	if not zwaveMonitor then
		local min
		for dev_num, v in pairs(luup.devices) do
			if v.device_type == DEVTYPE_ZWAVEMONITOR then
				if not zwaveMonitor or zwaveMonitor > dev_num then
					zwaveMonitor = dev_num
				end
			end
		end
	end
...
function ConnectPeerDevice(peer_dev_num)
	...
	if v2.device_type == SCObj.DEVTYPE and v2.device_num_parent == 0 then
		...
		if v == nil then
			...
			VariableWatch("SceneController_LastSceneIDChange",SCTRL_SID,"LastSceneID",zwave_dev_num,"")
			VariableWatch("SceneController_LastSceneTimeChange",SCTRL_SID,"LastSceneTime",zwave_dev_num,"")
			if IsVeraPrimaryController() then
--HACK_HACVK				luup.variable_watch("SceneController_ConfiguredChanged",HADEVICE_SID,"Configured",zwave_dev_num)
				if param.SCENE_CTRL_Retries > 0 then
...
function SceneController_LastSceneIDChange(lul_device, lul_service, lul_variable, lul_value_old, lul_value_new, context)
	...
	TempVariableUnwatch(SCTRL_SID, "LastSceneID", zwave_dev_num)
	luup.variable_set(SCTRL_SID, "LastSceneID", "", zwave_dev_num)
